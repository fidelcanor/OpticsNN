...
def init_weights(shape):
    ...
    weights = tf.random_normal(shape, stddev=.01)
    return tf.Variable(weights)

def init_bias(shape):
    """ Weight initialization """
    biases = tf.random_normal([shape], stddev=.01)
...
def get_data(data,percentTest=.2,random_state=42):
    ...
    for dataset in [X_train,X_val]:
        for instance in dataset:
            if i==0:
                trainX.append([np.reshape(np.linspace(400,800,num=201),(201,1)),instance])
            else:
                valX.append([np.reshape(np.linspace(400,800,num=201),(201,1)),instance])
        i+=1
...
def forwardprop(X, weights, biases, num_layers,dropout=False):
    ...
    for i in range(0, num_layers):
        if i ==0:
            htemp = tf.nn.conv2d(X,filter=weights[i],strides=[1,1,1,1],padding="VALID")
            htemp = tf.nn.avg_pool(htemp,ksize=[1,1,2,1],strides=[1,1,2,1],padding="VALID")
            htemp = tf.add(tf.nn.relu(htemp),biases[i])
            print(htemp)
        elif i==1:
            htemp = tf.nn.conv2d(htemp,filter=weights[i],strides=[1,1,1,1],padding="VALID")
            htemp = tf.nn.avg_pool(htemp,ksize=[1,1,2,1],strides=[1,1,2,1],padding="VALID")
            htemp = tf.add(tf.nn.relu(htemp),biases[i])
...
def main(data,reuse_weights,output_folder,weight_name_save,weight_name_load,n_batch,numEpochs,lr_rate,lr_decay,num_layers,n_hidden,percent_val,kernel_size,kernel_no):
    ...
    # Symbols
    X = tf.placeholder("float", shape=[None, 2, x_size, 1])
    ...
    for i in range(0,num_layers):
        if i == 0:
            weights.append(init_weights((2,kernel_size,1,kernel_no)))
            biases.append(init_bias(kernel_no))
        elif i==1:
            weights.append(init_weights((1,kernel_size,kernel_no,kernel_no)))
            biases.append(init_bias(kernel_no))
        elif i==2:
            a = int(0.5*(x_size-kernel_size+1))
            weights.append(init_weights((int(0.5*(a-kernel_size+1))*kernel_no,n_hidden)))
...
if __name__=="__main__":
    ...
    parser.add_argument("--data",type=str,default='data/CompleteDataFiles/4_layer_tio2_combined_06_21')
    parser.add_argument("--reuse_weights",type=str,default='True')
    parser.add_argument("--output_folder",type=str,default='results/4_Layer_TiO2_25Kernel_2DConvolution_4layers_850per/')
