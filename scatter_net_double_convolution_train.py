...
def forwardprop(X, weights, biases, num_layers,dropout=False):
    ...
    for i in range(0, num_layers):
        if i ==0:
            ...
            htemp = tf.nn.conv1d(htemp,filters=weights[i],stride=1,padding="VALID")
            htemp = tf.layers.average_pooling1d(htemp,2,2)
            htemp = tf.add(tf.nn.relu(htemp),biases[i])
...
if __name__=="__main__":
    ...
    parser.add_argument("--output_folder",type=str,default='results/4_Layer_TiO2_25Kernel_DoubleConvolution_4layers_850per/')
